...
class ImgFeature:
    ...
    def __init__(self):
        ...
        self.input_raw_sub = rospy.Subscriber("camera/color/image_raw", Image, self.image_raw_callback)
        self.input_segm_sub = rospy.Subscriber("segmentation/color/image_raw", Image, self.image_callback)
        self.depth_sub = rospy.Subscriber("camera/aligned_depth_to_color/image_raw", Image, self.imageDepthCallback)
        self.raw_image_pub = rospy.Publisher("labeled/color/image_raw", Image, queue_size=1)
        self.segm_image_pub = rospy.Publisher("labeled/color/image_segmented", Image, queue_size=1)
    ...
    def parse(self, img, img_raw):
        ...
        hz = 1  # ideja da se mijenjaju oznake svakih hz frameova, a ne svaki (vraca gresku nekad, pronaci bolje rjesenje)
        self.n += 1
        self.i += 1
        if self.n == hz:
            self.past_points = {}
                ...
                for c in contours:
                    ...
                    if M["m00"] > 300:
                        ...
                        cv2.putText(img, "{} ({} m)".format(name, self.get_mean_depth(cv_depth_image, cY, cX, 15)),
                                    (cX - 40, cY - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
